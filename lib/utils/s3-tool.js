...
function upload(file) {
  var filePath = path.join(_prependPath, file.path);
  var params = buildFileParameters(file.fullPath, {
    Key: filePath,
    ContentLength: file.stat.size
  });

  _promise = _promise.then(module.exports.uploadFile(_s3, _ui, file, params)).then(processResult);
}
 ...
 */
var _prependPath, _deferred, _promise, _count, _s3, _ui;
module.exports.uploadDirectory = function(s3, ui, dir, options) {
  _s3 = s3;
  _ui = ui;
  _prependPath = s3.config.prependPath || '';
