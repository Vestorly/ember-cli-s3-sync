...
module.exports.uploadFile = function(s3, ui, file, params) {
  ...
  return function() {
    ...
    return putObject(params).then(function(data) {
        ...
        return Promise.resolve({ error, file });
      });
  }
}
...
module.exports.uploadDirectory = function(s3, ui, dir, options) {
  let prependPath = s3.config.prependPath || '';
  ...
  return fileTool.readDirectory(dir).then(function(files) {
    ...
    return new Promise(resolve => {
      let processResult = function(result){
        if ( result.error ) {
          ui.writeLine(chalk.red(result.error.message));
          upload(result.file);
        } else if (--count <= 0 ) {
          resolve();
        }
      ...
      let upload = function(file) {
        let filePath = path.join(prependPath, file.path);
        let params = buildFileParameters(file.fullPath, {
          Key: filePath,
          ContentLength: file.stat.size
        });

        promise = promise.then(module.exports.uploadFile(s3, ui, file, params)).then(processResult);
      };
