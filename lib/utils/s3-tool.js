...
/**
 * A helper that returns the next element of an array on demand
 *
 * @method producer
 * @param {Array} arr
 * @return {Object} The current element being processed in the array
 */
...
module.exports.uploadDirectory = function(s3, ui, dir, options) {
  ...
  return fileTool.readDirectory(dir).then(function(files) {
    ...
    var gen = producer(source);

    var payload = gen.next();
    while ( !payload.done ) {
      var file = payload.value;
